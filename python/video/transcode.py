...
from defaults import DEFAULT_RENDER_PROFILE, DEFAULT_VIDEO_FORMAT, VIDEO_OUTPUT_ROOT
from files import create_output_path, delete_if_exists
import subprocess

def transcode_video(input_file, output_file=None, profile=DEFAULT_RENDER_PROFILE, filters=[], extension=DEFAULT_VIDEO_FORMAT):
    if output_file is None:
        output_file = create_output_path(input_file, VIDEO_OUTPUT_ROOT, 
...
def loop_video(input_file, output_file=None, loops=10):
    if output_file is None:
        output_file = create_output_path(input_file, VIDEO_OUTPUT_ROOT, extension='.mp4', prefix='LOOPED_')
    delete_if_exists(output_file)
    cmd = f'ffmpeg -stream_loop {loops} -i {input_file} -c copy {output_file}'
    subprocess.run(cmd, shell=True)

def screenshot_video(input_file, output_file=None, time=0):
    if output_file is None:
        output_file = create_output_path(input_file, VIDEO_OUTPUT_ROOT, extension='.png', prefix='SCREENSHOT_')
    delete_if_exists(output_file)
    cmd = f'ffmpeg -ss {time} -i {input_file} -vframes 1 {output_file}'
    subprocess.run(cmd, shell=True)


if __name__ == '__main__':
    input_file = '~/Downloads/Monica Rizzolli_Fuchsia_2021.mp4'
